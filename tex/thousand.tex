...
\href{https://www.ijcai19.org/call-for-paper-AI-for-improving-human-wellbeing.html}{\color{blue}IJCAI-special track}
\todo{@GUANJIE}
\begin{itemize}
    \item congestion and sustainable communities
    \item intelligent transportation balabalabala..
    \item implementation on large scale road net work is super hard
    \begin{itemize}
        \item heterogeneous intersections
        \item computation cost
        \item model generality
    \end{itemize}
\end{itemize}

The main contribution of this work is two-folds:
\begin{itemize}
\item proposed a novel decentralized network level control algorithm with a new reward definition, which could be theoretically justified by classical traffic signal control theory (maxpressure)

\item large scale application on more than a thousand lights in real-world network
\end{itemize}
}
...
% Story line
% \begin{itemize}
%     \item aim for large-scale network
%     \item no one has ever done before
%     \item concise state ; knowledge sharing (?)       
%     \item state action reward (large-scale)\\        
%     learn from MP (reward design with provable objective)
% \end{itemize}}

\nop{Urban traffic congestion has been a major obstacle to sustainable development. Due to the rapid urbanization which results in an explosive increase in household owning cars, traffic congestion, along with health-damaging air pollution, high stress of commuters and economic loss, \hl{now stands at all time high}.}
...
A natural way is to deal with the individual intersection signal control and to add coordination upon it. However, two major challenges occur following this line of method. 

First, training thousands of RL agents independently is expensive and time-consuming. The thousands of RL agents, though controlling different traffic flows, are essentially following similar control logic. Their learned knowledge should be shared to enhance the speed of training. How can we design a parameter sharing diagram to do so?

\nop{
However, training thousands of RL agents independently is expensive and time-consuming. When similar agents are learning similar behaviors, their learned knowledge should be shared to enhance the speed of training and to utilize resource. Homogeneous agents could benefit from other agents' experiences and learned policies by learning in a parameter sharing paradigm~\cite{gupta2017cooperative}. Unlike a small-region or grid road net, a real world road net with thousands of intersections covers heterogeneous types of intersections (i.e., the number of entering approaches and the number of lanes) and traffic signal settings, how should we design a generic model for all the intersections?
}

% However, considering that a large region may cover heterogeneous types of intersections (i.e., number of entering approaches and number of lanes) and traffic signal settings, how should we design a generic model for all the intersections?
% Hence, to speed up the training process, knowledge sharing is facilitated among all the agents. 
% we design and implement a generic model for all the intersections in order to facilitate parameter sharing among all the agents. 
% However, simply letting each agent learn independently means allowing agents to repeat similar mistakes (i.e., setting bad signal), which causes real traffic congestion. Therefore, a better solution is to enable the knowledge sharing among intersections. 
% However, considering that a large region may cover heterogeneous types of intersections (i.e., number of entering approaches and number of lanes) and traffic signal settings, how should we design a generic model for all the intersections?

\nop{
Second, when dealing with multi-intersection traffic signal control, it is hard to assign the region-wise reward function to each intersection. Previous works~\cite{ElAb12,van2016coordinated,nishi2018traffic} usually adopt common transportation measurements as the reward function, e.g., average wait time of vehicles~\cite{nishi2018traffic} and delay~\cite{ElAb12,van2016coordinated}, but there is no guarantee that the overall objective is optimized by letting each agent maximize its own expected reward. How should we design the reward for each individual agent to help them learn the implicit coordination and to optimize towards a global objective? }
...
% \item The superior performance of our learned policy could be validated using Macroscopic Fundamental Diagram.
...
% Extensive experiments on real world data are conducted to demonstrate the effectiveness of the algorithm.
\end{itemize}

\section{Related Work}


% Pre-timed control $\rightarrow$ Actuated $\rightarrow$ Adaptive $\rightarrow$ Learning
% \textbf{1.Learning is better}\\
% \textbf{2.No one has ever done more than a thousand lights}
% (MFD parameter control is heuristic)\\
% \textbf{3.What can we learn from transportion}(draw a connection)\\
% Maxpressure contorl $\rightarrow$ Macroscopic Theory \\
% % \begin{itemize}
% % \item transportation fields
% % \begin{itemize}
% % 	\item FT
% % 	\item Formula
% % 	\item Actuated control

% % \end{itemize}
% % \item CS
% % \begin{itemize}
% % \item MARLIN-ATSC   59 intersections in downtown Toronto
% % \item NIPS wiering
% % \item IntelliLight
% % \end{itemize}
...
% % \end{itemize}
% Isolated inersection control $\rightarrow$ region level control
...
\begin{figure}[htbp]
\centering
\includegraphics[width=1.0\textwidth]{figures/road_net.pdf}
...
% {p{0.1\textwidth}p{0.07\textwidth}p{0.07\textwidth}p{0.07\textwidth}|p{0.07\textwidth}p{0.07\textwidth}p{0.07\textwidth}p{0.07\textwidth}p{0.07\textwidth}}
\toprule
